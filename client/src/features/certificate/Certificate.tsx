...
export default function EngagementConfirmation({
	...
	const updateEngagement = (id: string, key: string, value: any) => {
		const updated = engagements.map((engagement) => {
			if (engagement.id === id) {
				const newEngagement = { ...engagement, [key]: value };
	...
	const generatePdfBlob = async () => {
		...
		for (const engagement of engagements) {
			if (
				...
				alert(
					"Please complete all required fields for each engagement before generating the PDF.",
				);
		...
		const birthDate = new Date(memberData.date_of_birth).toLocaleDateString(
			"de-DE",
		);
...
Each member shapes their TUM.ai journey by joining one of the departments to contribute to our growth, launching new initiatives, and participating in hands-on offerings.
	...
	const handleSendPdfEmail = async () => {
		...
		if (!recipientEmail) {
			alert(
				"Could not find recipient email. Please ensure your user data includes an email address.",
			);
	...
	return (
		<div
			style={{ color: "white", maxWidth: 700, margin: "auto", padding: "1rem" }}
		>
			<h1 style={{ textAlign: "center" }}>
				Confirmation of Voluntary Engagement
			</h1>
			...
			<p>
				This form will generate a personalized <strong>PDF certificate</strong>{" "}
				confirming your voluntary engagement with{" "}
				<strong>{institutionName}</strong>. Please enter{" "}
				<strong>accurate information</strong> for each period you were actively
				involved.
			</p>
			...
			<p>
				...
				, born on{" "}
				<strong>
					{new Date(memberData.date_of_birth).toLocaleDateString("de-DE")}
				</strong>
				, has voluntarily engaged with our institution{" "}
						...
						<div>
							<label>
								...
								<input
									...
									onChange={(e) =>
										updateEngagement(engagement.id, "startDate", e.target.value)
									}
									required
								/>
							</label>
							...
							<div>
								<label>
									...
									<input
										...
										onChange={(e) =>
											updateEngagement(engagement.id, "endDate", e.target.value)
										}
