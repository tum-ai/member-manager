...
export default function MemberForm({ user }: MemberFormProps) {
	...
	useEffect(() => {
		const handleSepaUpdate = (event: Event) => {
			...
			if (
				...
				setSepa((prev) => ({
					...prev,
					mandate_agreed: customEvent.detail.mandate_agreed,
				}));
			}
		};
		...
		const handlePrivacyUpdate = (event: Event) => {
			...
			if (
				...
				setSepa((prev) => ({
					...prev,
					privacy_agreed: customEvent.detail.privacy_agreed,
				}));
			}
		};
	...
	function handleSepaCheckbox(e: React.ChangeEvent<HTMLInputElement>) {
		...
		if (!sepa.mandate_agreed && checked) {
			...
			window.dispatchEvent(
				new CustomEvent("sepa-updated", { detail: { mandate_agreed: false } }),
			);
		} else if (!sepa.mandate_agreed && !checked) {
				...
				<>
					<div style={{ display: "flex", gap: "2rem", flexWrap: "wrap" }}>
						...
						<div style={{ flex: 1, minWidth: "300px" }}>
							...
							<p>
								<strong>IBAN:</strong> {sepa.iban || "-"}
							</p>
							<p>
								<strong>BIC:</strong> {sepa.bic || "-"}
							</p>
							<p>
								<strong>Bank name:</strong> {sepa.bank_name || "-"}
							</p>
							<p>
								<strong>SEPA Mandate:</strong>{" "}
								{sepa.mandate_agreed ? "Accepted" : "Not Accepted"}
							</p>
							<p>
								<strong>Privacy Policy:</strong>{" "}
								{sepa.privacy_agreed ? "Accepted" : "Not Accepted"}
							</p>
						</div>
					</div>
				...
				<form onSubmit={updateData}>
					<div style={{ display: "flex", gap: "2rem", flexWrap: "wrap" }}>
						<div style={{ flex: 1, minWidth: "300px" }}>
							...
							{personalFields
								.filter(
									(f) => !["title", "salutation", "active"].includes(f.name),
								)
								.map(({ label, name, type }) => (
									<label
										key={name}
