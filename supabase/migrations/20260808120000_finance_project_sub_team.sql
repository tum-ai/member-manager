-- T-Konto workbench, FR-L4: a project can be created *inside* a sub-team folder
-- in the T-view, so the project itself has to remember which sub-team it hangs
-- under. Until now a sub-team existed only as an attribute of a cost-location
-- mapping (`finance_department_mappings.sub_team`), which can only place
-- *unallocated postings* — never a project.
--
-- Precedence (also documented in docs/finance-cost-location-mapping.md):
--   * mapping.sub_team    → where an unallocated posting lands
--   * project.sub_team    → where a project folder hangs
-- The two are independent; a project's sub-team is set by hand and does not
-- follow the cost locations of the postings allocated to it.
...
-- Sub-team folders are rendered per department, so the T-account groups by
-- (department, sub_team). Partial: most projects hang directly off a department.
create index if not exists "finance_projects_sub_team_idx"
on "public"."finance_projects" ("department", "sub_team")
where "sub_team" is not null;

-- `update_finance_project` gains p_sub_team. Postgres would otherwise keep both
-- arities as ambiguous overloads, so the old signature is dropped explicitly
...
-- Retire the pre-sub_team arity so callers cannot resolve to it by accident.
drop function if exists "public"."update_finance_project"(
    uuid,
    uuid,
    text,
    text,
