...
-- Finance: BuchhaltungsButler cost location → department + sub-team mappings.
...
-- a sub-team-owned project, an active plan item with a planned VAT rate, a
-- partially matched one (whose match moves it to 'committed'), and a disabled
-- one. BB-1077 is the mock BuchhaltungsButler posting
-- "Onboarding SS Catering" (cost location 111 → Community / Onboarding,
-- -840 EUR, 2026-04-18). The id is generated by the mock posting generator in
-- server/src/lib/buchhaltungsbutlerPostings.ts, so it must be re-checked there
-- whenever postings are added before it — an id that points at the wrong
-- posting seeds a state the API would never create.
--
...
values
    ('c0000000-0000-4000-8000-000000000011', 'BB-1077', 'Community',
...
insert into public.finance_plan_items
    (id, department, period_type, period_key, label, category, direction,
     ...
     vat_rate)
values
    -- Partially matched by BB-1077 (840 of 1000) → status 'committed'.
...
values
    ('c0000000-0000-4000-8000-000000000031',
     'c0000000-0000-4000-8000-000000000021', 'BB-1077', 840, 'manual')
on conflict (id) do nothing;
