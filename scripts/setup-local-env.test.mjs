...
test("buildClientEnv preserves hand-configured PostHog values", () => {
	const env = buildClientEnv({
		apiUrl: "http://127.0.0.1:54321",
		anonKey: "anon-123",
		existingEnv: [
			"VITE_SUPABASE_URL=http://stale",
			"VITE_POSTHOG_KEY=phc_local_key",
			"VITE_POSTHOG_HOST=/ingest",
		].join("\n"),
	});

	assert.match(env, /^VITE_POSTHOG_KEY=phc_local_key$/m);
	assert.match(env, /^VITE_POSTHOG_HOST=\/ingest$/m);
	// Supabase values are still regenerated, not carried over.
	assert.match(env, /^VITE_SUPABASE_URL=http:\/\/127\.0\.0\.1:54321$/m);
});

test("buildClientEnv omits PostHog keys that were never configured", () => {
	const env = buildClientEnv({
		apiUrl: "http://127.0.0.1:54321",
		anonKey: "a",
	});

	assert.doesNotMatch(env, /VITE_POSTHOG_/);
});
