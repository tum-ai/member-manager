...
export function buildSystemPrompt(
	...
	// Tactical guidance contributed by whichever pillars are currently loaded.
	const guidance = loaded
		.map((id) => registry.get(id)?.promptGuidance?.trim())
		.filter((g): g is string => Boolean(g));
	const guidanceBlock = guidance.length
		? `\n\nWhen working in the loaded areas:\n${guidance
				.map((g) => `- ${g}`)
				.join("\n")}`
		: "";
...
- Be efficient: prefer ONE broad action that covers the whole need over many narrow lookups, and never enumerate a list of entities to look each up separately.
